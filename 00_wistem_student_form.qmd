...
**Distribution:** It is known that students collaborated on this question at times so that's why some careers appear more often than others. However, in this section, every career option was considered only once, no matter how many students. actually wrote it down either on the Pre-Test or Post-Test. The table below groups all of the careers students wrote down by what major they would relate to in college. Out of all the 77 unique careers seen in the Pre-Test and the 56 found in the Post-Test, most careers would fall under "Medical" or "Pre-Med" majors in undergrad. 
```{r}
...
major_career <- full_join(pre_major_career, post_major_career) %>%
  ...
  rename(
    ...
    `Percent (Pre)` = pre_percent, 
    `Precent (Post)` = post_percent
  ) %>%
  kbl(caption = "Careers Grouped by Major Category") %>%
  kable_styling()
major_career
```
...
## Combine
stem_cat_career_all <- full_join(pre_stem_cat_career, post_stem_cat_career) %>% 
  pivot_longer(cols = c(pre, post), names_to = "Type", values_to = "Count") %>%
  group_by(Type) %>%
...
stem_cat_career_all$Category <- factor(stem_cat_career_all$Category, levels = c("Science", "Technology", "Engineering", "Arts", "Mathematics"))
stem_cat_career_all$Type <- factor(stem_cat_career_all$Type, levels = c("pre", "post"))

# Compare
## Graph
colors <- c("steelblue1", "tomato")

stem_cat_plot <- ggplot(data = stem_cat_career_all, mapping = aes(x = Category, y = Percentage, fill = Type)) + 
  geom_col(position = 'dodge') + 
  geom_text(data = stem_cat_career_all, mapping = aes(x = Category, y = Percentage, label = sprintf("%.2f%%", Percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Career Category", 
    y = "Percentage", 
    title = "Careers Grouped by Their STEM Category", 
    subtitle = "Per Survey Type", 
    caption = "Source: WiSTEM 2022-2023"
  ) + 
  theme_minimal() + 
  theme(plot.title = element_text(face = "bold"))
...
::: {.callout-note icon=false collapse="true"}
## Table for Careers Grouped by STEM Type
```{r}
## Table
stem_cat_career <- stem_cat_career_all %>%
  kbl(caption = "Careers Grouped by STEM Type Per Survey Type") %>%
  kable_styling()
stem_cat_career
```
...
pre_wh_order$survey_type <- factor(pre_wh_order$survey_type, levels = c("Strongly disagree", "Disagree", "Neutral", "Agree", "Strongly agree"))
...
percentage_data <- data %>%
  ...
  mutate(percentage = count / sum(count) * 100)
percentage_data$survey_type <- factor(percentage_data$survey_type, levels = c("Pre", "Post"))

one_wh_plot <- ggplot(data = percentage_data, mapping = aes(x = work_hard, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = work_hard, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Work Hard Response", 
    y = "Percentage", 
...
one_know_plot <- ggplot(data = percentage_data, mapping = aes(x = knowledge, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = knowledge, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Knowledge Response", 
    y = "Percentage", 
...
one_course_plot <- ggplot(data = percentage_data, mapping = aes(x = courses, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = courses, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Courses Response", 
    y = "Percentage", 
    title = "Confident in My Ability to do Well in STE(A)M Courses", 
    subtitle = "Per Survey Type", 
    caption = "Source: WiSTEM 2022-2023"
  ) + 
  theme_minimal() + 
...
one_succ_plot <- ggplot(data = percentage_data, mapping = aes(x = succesful, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = succesful, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
...
one_imp_plot <- ggplot(data = percentage_data, mapping = aes(x = important, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = important, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Important Response", 
    y = "Percentage", 
    title = "STE(A)M People Solve Important Problems", 
    subtitle = "Per Survey Type", 
    caption = "Source: WiSTEM 2022-2023"
  ) + 
  theme_minimal() + 
...
one_mem_plot <- ggplot(data = percentage_data, mapping = aes(x = member, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = member, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Member Response", 
    y = "Percentage", 
    title = "I'm a Member of a STE(A)M Community", 
    subtitle = "Per Survey Type", 
    caption = "Source: WiSTEM 2022-2023"
  ) + 
  theme_minimal() + 
...
one_car_plot <- ggplot(data = percentage_data, mapping = aes(x = career, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = career, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Career Response", 
    y = "Percentage", 
    title = "A STE(A)M Career is a Good Fit for Me", 
    subtitle = "Per Survey Type", 
    caption = "Source: WiSTEM 2022-2023"
  ) + 
  theme_minimal() + 
...
one_iden_plot <- ggplot(data = percentage_data, mapping = aes(x = identity, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = identity, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Identity Response", 
    y = "Percentage", 
    title = "`Future STE(A)M Professional` is a Part of My Identity", 
    subtitle = "Per Survey Type", 
    caption = "Source: WiSTEM 2022-2023"
  ) + 
  theme_minimal() + 
...
one_mean_plot <- ggplot(data = percentage_data, mapping = aes(x = meaningful, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = meaningful, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Meaningful Response", 
    y = "Percentage", 
    title = "I Would Make Meaningful Contribution in STE(A)M",
    subtitle = "Per Survey Type", 
    caption = "Source: WiSTEM 2022-2023"
  ) + 
  theme_minimal() + 
...
one_every_plot <- ggplot(data = percentage_data, mapping = aes(x = everyday, y = percentage, fill = survey_type)) + 
  geom_col(position = 'dodge') + 
  geom_text(mapping = aes(x = everyday, y = percentage, label = sprintf("%.2f%%", percentage)), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) + 
  scale_fill_manual(labels = c("Pre", "Post"), values = colors) + 
  labs(
    x = "Everday Response", 
    y = "Percentage", 
    title = "I Understand Everyday STE(A)M Work", 
    subtitle = "Per Survey Type", 
    caption = "Source: WiSTEM 2022-2023"
  ) + 
  theme_minimal() + 
